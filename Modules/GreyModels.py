...
from CommonOperations import _res_funcs_list
...
class GM():
    ...
    def predict(self, t):
        ...
        x1_pred = _res_funcs_list.res_funcs[self.mdl_name].compute(self.params, all_t, self.x_orig[0])
...
class BernoulliGM():
    ...
    def predict(self, t):
        ...
        x1_pred = _res_funcs_list.res_funcs[self.mdl_name].compute(self.params, all_t, self.x_orig[0], self.n)
        x_pred = Accumulation.ago(x1_pred, self.x_orig[0], False)
        return x_pred
...
class NDGM():
    ...
    def predict(self, t):
        all_t = np.arange(0, np.max(t))
        x1_pred = _res_funcs_list.res_funcs[self.mdl_name].compute(self.params, all_t, self.x1)
        x_pred = Accumulation.ago(x1_pred, self.x_orig[0], False)
        return x_pred
...
class GMN():
    ...
    def predict(self, t_x):
        all_t = t_x[0:, 0]
        x1 = t_x[0:, 1:]
        x1_pred = _res_funcs_list.res_funcs[self.mdl_name].compute(self.params, all_t, x1)
